...
<div class="min-h-screen bg-gray-50 py-12" x-data="chatSystem()">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Chat Container -->
        <div class="bg-white rounded-3xl md:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 flex flex-col h-[70vh] md:h-[650px]">
            <!-- Messages Area -->
            <div id="chat-window" class="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 md:space-y-6 scroll-smooth custom-scrollbar">
                @foreach($messages as $message)
                <div class="flex {{ $message->sender_id === Auth::id() ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[85%] md:max-w-[75%] {{ $message->sender_id === Auth::id() ? 'order-1' : 'order-2' }}">
                        <div class="px-4 md:px-5 py-2.5 md:py-3 rounded-2xl md:rounded-3xl text-sm font-medium shadow-sm {{ $message->sender_id === Auth::id() ? 'bg-primary text-white rounded-tr-none' : 'bg-slate-50 text-slate-700 rounded-tl-none border border-slate-100' }}">
                            {{ $message->content }}
                        </div>
                ...
                <template x-for="msg in dynamicMessages" :key="msg.id">
                    <div class="flex" :class="msg.sender_id === {{ Auth::id() }} ? 'justify-end' : 'justify-start'">
                        <div class="max-w-[85%] md:max-w-[75%] shadow-sm">
                            <div class="px-4 md:px-5 py-2.5 md:py-3 rounded-2xl md:rounded-3xl text-sm font-medium" 
            ...
            <div class="p-4 md:p-6 border-t border-slate-50 bg-slate-50/30">
                <form @submit.prevent="sendMessage()" class="flex gap-2 md:gap-4">
                    <input type="text" x-model="newMessage" :disabled="loading" 
                           class="flex-1 border-none bg-white rounded-xl md:rounded-2xl px-4 md:px-6 py-3 md:py-4 shadow-sm focus:ring-2 focus:ring-primary/20 text-sm font-medium placeholder:text-slate-300 transition-all" 
                           placeholder="Mesajınızı yazın...">
                    <button type="submit" :disabled="loading || !newMessage.trim()" 
                            class="bg-slate-900 text-white px-5 md:px-8 rounded-xl md:rounded-2xl font-black text-xs uppercase tracking-widest border-2 border-white/10 hover:border-white transition-all shadow-xl shadow-slate-900/20 active:scale-95 disabled:opacity-50 flex items-center justify-center">
                        <span x-show="!loading" class="hidden md:inline">Gönder</span>
                        <span x-show="!loading" class="md:hidden"><i class="fa-solid fa-paper-plane text-base"></i></span>
