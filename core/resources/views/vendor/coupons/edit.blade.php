...
<div class="min-h-screen bg-gray-50/50 pb-20" x-data="couponForm({ 
    ...
    min_amount: '{{ old('min_amount', $coupon->min_amount) }}',
    expires_at: '{{ old('expires_at', optional($coupon->expires_at)->format('Y-m-d')) }}' 
})">
    <!-- Header -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-black text-gray-900 tracking-tight">Kuponu Düzenle</h1>
                        <p class="text-xs text-gray-500 font-medium">{{ $coupon->code }} kuponunu güncelliyorsunuz</p>
                    </div>
                </div>
                
                <div class="hidden md:flex items-center gap-2">
                    <span class="px-3 py-1 rounded-full bg-amber-50 text-amber-600 text-xs font-bold uppercase tracking-wider">Düzenleme Modu</span>
                </div>
            </div>
        </div>
    </div>
                                            ...
                                            <template x-for="date in noOfDays">
                                                <div @click="getDateValue(date)"
                                                     ...
                                                     :class="{
                                                         ...
                                                         'hover:bg-gray-50 text-gray-700 hover:text-indigo-600 hover:scale-105': !isSelected(date),
                                                         'text-gray-300 decoration-slice': false
                                                     }">
                ...
                <div class="lg:col-span-5 relative hidden lg:block">
                     <div class="sticky top-32">
                        <div class="relative mb-6">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Kuponunuz müşterilere böyle görünecek.</p>
                        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('couponForm', (initialData) => ({
            ...
            isPast(date) {
                // In edit mode, allow past dates for now so user can see/edit expired coupons
                const d = new Date(this.year, this.month, date);
                const today = new Date();
                today.setHours(0,0,0,0);
                // return d < today; // disabled restriction for edit
                return false; 
            },
