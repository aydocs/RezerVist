    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-8" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Yeni Kaynak</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            <div class="p-8 border-b border-slate-50">
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Yeni Masa/Yer Ekle</h1>
                <p class="mt-1 text-sm text-gray-500 font-medium">Masa veya alan bilgilerini tanımlayın.</p>
            </div>

            <form action="{{ route('vendor.resources.store') }}" method="POST" class="p-8 space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" required value="{{ old('name') }}" class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 font-bold text-slate-900 transition-all" placeholder="Örn: Masa 1, Loca A, VIP Oda">
                        @error('name') <p class="mt-1 text-xs text-rose-500 font-bold ml-1">{{ $message }}</p> @enderror
                    </div>

                    <!-- Location Selection -->
                    <div>
                        <label for="location_id" class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">ŞUBE (OPSİYONEL)</label>
                    ...
                    <div>
                        ...
                        <select name="category" id="category" class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 font-bold text-slate-900 transition-all appearance-none">
                            ...
                            <option value="Bahçe" {{ old('category') == 'Bahçe' ? 'selected' : '' }}>Bahçe</option>
                            <option value="VIP" {{ old('category') == 'VIP' ? 'selected' : '' }}>VIP / Özel Oda</option>
                            <option value="Teras" {{ old('category') == 'Teras' ? 'selected' : '' }}>Teras</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 font-bold text-slate-900 transition-all appearance-none">
                            ...
                            <option value="VİP Oda">VİP Oda</option>
                            <option value="Bahçe Masası">Bahçe Masası</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="capacity" id="capacity" required min="1" value="{{ old('capacity', 2) }}" class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 font-bold text-slate-900 transition-all">
                    ...
                    <div class="md:col-span-2 p-6 bg-slate-50 rounded-2xl">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <input type="checkbox" name="requires_inventory" value="1" {{ old('requires_inventory') ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex-1">
                                ...
                                <input type="number" name="stock" id="stock" value="{{ old('stock', 0) }}" class="w-full px-6 py-4 bg-white border-none rounded-2xl focus:ring-2 focus:ring-primary/20 font-bold text-slate-900 transition-all" placeholder="Örn: 50">
                ...
                <div class="pt-6 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <button type="submit" class="px-10 py-4 bg-primary text-white rounded-[2rem] font-black text-sm shadow-xl shadow-purple-200 hover:bg-purple-700 transition-all">
                        Kaydet
                    </button>
                </div>
            </form>
