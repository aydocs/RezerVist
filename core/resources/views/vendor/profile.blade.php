...
@section('title', 'İşletme Profili - ' . $globalSettings['site_name'])
...
<div class="min-h-screen bg-gray-50 pt-24 pb-12">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">İşletme Profili</h1>
                <p class="text-gray-500 mt-2">Kişisel bilgilerinizi ve işletme bağlantınızı yönetin.</p>
            </div>
            <div class="flex flex-wrap gap-3">
                @if(Auth::user()->business)
                <a href="{{ route('business.show', Auth::user()->business) }}" target="_blank" class="inline-flex items-center justify-center px-5 py-2.5 border border-gray-200 text-sm font-medium rounded-xl text-gray-700 bg-white hover:bg-gray-50 transition duration-200 shadow-sm">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    Sayfayı Görüntüle
                </a>
                @endif
                <a href="{{ route('vendor.dashboard') }}" class="inline-flex items-center justify-center px-5 py-2.5 border border-transparent text-sm font-medium rounded-xl text-primary bg-primary/10 hover:bg-primary/20 transition duration-200">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden sticky top-28">
                    ...
                    <div class="px-6 pb-6 text-center -mt-12 relative">
                        ...
                        <div class="mt-6 border-t border-gray-100 pt-6 text-left">
                            ...
                            <!-- Business Link -->
                            @if($user->business)
                            <a href="{{ route('business.show', $user->business) }}" class="flex items-center text-primary text-sm hover:underline">
                                <svg class="w-5 h-5 mr-3 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                                {{ $user->business->name }}
                            </a>
                            @endif
                        </div>
                    ...
                    <div class="p-6">
                        <form action="{{ route('profile.update') }}" method="POST">
                            ...
                            <div class="flex justify-end pt-4">
                                <button type="submit" class="inline-flex items-center justify-center px-8 py-3 border border-transparent text-sm font-bold rounded-xl text-white bg-gray-900 hover:bg-gray-800 transition duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
