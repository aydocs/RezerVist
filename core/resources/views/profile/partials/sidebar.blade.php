...
<div class="lg:hidden mb-6">
    <div class="flex gap-3 overflow-x-auto hide-scrollbar pb-1 snap-x -mx-4 px-4">
        <a href="{{ route('profile.edit') }}" 
           class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border transition-all
           {{ request()->routeIs('profile.edit') ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-gray-600 border-gray-100 hover:border-primary/30' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
            <span class="text-sm font-bold">Profil</span>
        </a>
        <a href="{{ route('profile.reservations') }}" 
           class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border transition-all
           {{ request()->routeIs('profile.reservations') ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-gray-600 border-gray-100 hover:border-primary/30' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
            <span class="text-sm font-bold">Rezervasyonlarım</span>
        </a>
        <a href="{{ route('profile.favorites') }}" 
           class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border transition-all
           {{ request()->routeIs('profile.favorites') ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-gray-600 border-gray-100 hover:border-primary/30' }}">
            <i class="fa-solid fa-heart text-[14px]"></i>
            <span class="text-sm font-bold">Favoriler</span>
        </a>
        <a href="{{ route('profile.wallet.index') }}" 
           class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border transition-all
           {{ request()->is('profile/wallet*') ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-gray-600 border-gray-100 hover:border-primary/30' }}">
            <i class="fa-solid fa-credit-card"></i>
            <span class="text-sm font-bold">Cüzdan</span>
        </a>
        <a href="{{ route('messages.index') }}" 
           class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border transition-all
           {{ request()->routeIs('messages.*') ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-white text-gray-600 border-gray-100 hover:border-primary/30' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
            <span class="text-sm font-bold">Mesajlar</span>
        </a>
        @if(Auth::user()->role === 'business')
            <a href="{{ route('vendor.dashboard') }}" 
            class="flex items-center gap-2 px-6 py-3 rounded-2xl whitespace-nowrap snap-start border bg-purple-600 text-white border-purple-600 shadow-lg shadow-purple-600/20">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                <span class="text-sm font-bold">İşletme Paneli</span>
            </a>
        @endif
    </div>
</div>
